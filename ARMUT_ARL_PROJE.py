...
df_ap= pd.pivot_table(df,index="SepetID",columns="Hizmet",aggfunc="size",fill_value=0)


df_ap =df_ap.applymap(lambda x: 1 if x>0 else 0)
...
frequent_hizmet =apriori(df_ap,min_support=0.01,use_colnames=True)

frequent_hizmet.sort_values("support", ascending=False)

rules = association_rules(frequent_hizmet,
                          metric="support",
                          min_threshold=0.01)
...
def hizmetler(dataframe, item_set):
    product_name = dataframe[dataframe["Hizmet"] == item_set].values[-1].tolist()
    print(product_name)
